...
1인 브랜드의 **네이버 스마트스토어 · 쿠팡 · 오프라인** 판매 데이터를 자동 수집하고, AI가 종합 분석하여 월간 비즈니스 인사이트를 생성하는 풀스택 웹 애플리케이션입니다.
...
### Data Integration — 멀티채널 데이터 통합

네이버·쿠팡·오프라인 3개 채널은 각각 **서로 다른 비용 구조와 상품명 체계**를 가지고 있습니다.
...
DB 없이 `data/reports/YYYY-MM.json` 파일로 저장합니다. 1인 운영 로컬 도구이므로 DB 설치 복잡도를 제거하고, 파일 하나에 해당 월의 모든 데이터(매출·비용·랭킹·인사이트)를 self-contained로 담아 백업·이동이 간편합니다.

`data/` 디렉토리는 구글 드라이브 폴더에 심볼릭 링크로 연결되어 있어, 데이터 유실 없이 자동으로 클라우드에 백업됩니다.
